...
module.exports.load = async function (app, db) {
  ...
  app.get("/create", async (req, res) => {
    ...
    if (newsettings.api.client.allow.server.create == true) {
      queue.addJob(async (cb) => {
        ...
        if (!cacheaccount) {
          cb()
          return res.send('Reliactyl failed to find an account on the configured panel, try relogging')
        }
        req.session.pterodactyl = cacheaccount.attributes;
  ...
  app.get("/modify", async (req, res) => {
    ...
    if (newsettings.api.client.allow.server.modify == true) {
      ...
      if (ram || disk || cpu) {
        // let newsettings = JSON.parse(fs.readFileSync("./settings.json").toString());
        // let newsettings = settings; // Shadowed, but same value.
        // We can just use the outer newsettings or settings.
